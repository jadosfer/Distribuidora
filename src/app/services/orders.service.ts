...
export class OrdersService implements OnDestroy, OnInit, OnChanges {
  getSellerFromForm(sellerForm: string) {
    for (let i = 0; i < this.sellers.length; i++) {
      if (
        this.sellers[i].payload
          .val()
          .name.toLowerCase()
          .includes(sellerForm.toLowerCase())
      )
        return this.sellers[i].payload.val().name;
    }
    return null;
  }
  ...
  orderNumber: any = [];
  fullPaymentDateSet: boolean;

  clientsPaginator: { pageIndex: number; pageSize: number; length: number } = {
    pageIndex: 0,
    pageSize: 10,
    length: 0,
  };
  ...
  constructor(
    ...
    this.subscription2 = this.auth.appUser$.subscribe((appUser) => {
      ...
      this.subscription3 = this.productService
        .getAll()
        .subscribe((products) => {
          ...
          this.subscription5 = this.getOrder().subscribe((order) => {
            ...
            for (let i = 0; i < this.order.length; i++) {
              if (
                this.order &&
                this.appUser &&
                this.order[i].payload.val().sellerName == this.appUser?.name
              ) {
                this.orderIndex = i;
    ...
    this.subscription7 = this.getAllOrders().subscribe((orders) => {
      ...
      // // Resta 15 días a la fecha actual
      // fechaActual.setDate(fechaActual.getDate() - 15);
      // // Obtiene el timestamp en milisegundos
      // const timestamp = fechaActual.getTime();

      // // Obtener la fecha y hora actual en milisegundos (timestamp)
      // const timestampActual = Date.now();

      // // Crear una fecha para mañana
      // const fechaManana = new Date();
      // fechaManana.setDate(fechaManana.getDate() + 1);
      // const timestampManana = fechaManana.getTime();

      // for (let i = 0; i < this.orders.length; i++) {
      //   if (this.orders[i].payload.val().fullPaymentDate > timestampManana) {
      //     console.log('order', this.orders[i].payload.val());
      //     this.updateOrder(this.orders[i].key, {
      //       ...this.orders[i].payload.val(),
  ...
  isProductStock(product: any) {
    for (let i = 0; i < this.products.length; i++) {
      if (
        this.products[i].payload.val().title == product.product.title &&
        this.products[i].payload.val().stock < product.quantity
      )
        return false;
    }
    return true;
  ...
  updateOrder(key: any, order: any) {
    return this.db.object('/orders/' + key).set(order);
  }

  removeOrderByKey(key: string) {
  ...
  async removeOrder(order: any) {
    ...
    this.reorderPaymentDates(order.payload.val().clientFantasyName);
  ...
  updateOrderItemQuantity(
    ...
    if (order[orderIndex]) {
      this.db.object('/order/' + order[orderIndex].key).update({
        ...order[orderIndex].payload.val(),
        orderItemsCount:
          order[orderIndex].payload.val().orderItemsCount + change,
      });
    }
    for (let i = 0; i < this.products.length; i++) {
      if (
        product.productId ==
        order[orderIndex].payload.val().products[i].productId
      ) {
        this.db
          .object('/order/' + order[orderIndex].key + '/products/' + i)
          .update({
            quantity:
              order[orderIndex].payload.val().products[i].quantity + change,
          });
        break;
      }
    }
  ...
  discount(order: any, product: any, discount: number, orderIndex: number) {
    ...
    for (
      let i = 0;
      i < order[this.orderIndex].payload.val().products.length;
      i++
    ) {
      if (
        product.productId ==
        order[this.orderIndex].payload.val().products[i].productId
      ) {
        this.db
          .object('/order/' + order[orderIndex].key + '/products/' + i)
          .update({
            discountPrice:
              order[this.orderIndex].payload.val().products[i].price *
              (1 - discount / 100),
            discount: discount,
          });
        break;
  ...
  createOrder(
    ...
    for (let i = 0; i < products.length; i++) {
      if (parseInt(products[i].quantity) != 0) {
        //solo guardo los prod con quant > 0
        prods.push({
          ...
          product: {
            ...
            prodsCategory: products[i].product.prodsCategory,
          },
        });
        amount +=
          parseInt(products[i].quantity) *
          parseFloat(products[i].discountPrice) *
          (1 + iva / 100);
    ...
    let result = this.db.list('/orders/').push({
      ...
      orderNumber: this.orderNumber[0].payload.val().orderNumber
        ? this.orderNumber[0].payload.val().orderNumber
        : 0,
    });
  ...
  restoreOrderAmount(payment: any) {
    ...
    for (let i = 0; i < this.orders.length; i++) {
      if (
        payment.payload &&
        payment.payload.val().orderNumber > 0 &&
        this.orders[i].payload.val().orderNumber ==
          payment.payload.val().orderNumber
      ) {
        if (
          parseFloat(this.orders[i].payload.val().amount) -
            parseFloat(this.orders[i].payload.val().debt) <=
          payment.payload.val().amount
        ) {
          rest =
            rest -
            (parseFloat(this.orders[i].payload.val().amount) -
              parseFloat(this.orders[i].payload.val().debt));
          let debt =
            Math.round(parseFloat(this.orders[i].payload.val().amount) * 10) /
            10;
          this.updateOrder(this.orders[i].key, { debt: debt });
          if (rest > 10)
            this.restoreOrderAmount({
              orderNumber: 0,
              amount: rest,
              client: payment.payload.val().client,
            });
          break;
    ...
    for (let i = 0; i < this.orders.length; i++) {
      if (
        this.orders[i].payload
          .val()
          .clientFantasyName?.toLowerCase()
          .includes(payment.client.toLowerCase())
      ) {
        if (
          parseFloat(this.orders[i].payload.val().amount) &&
          parseFloat(this.orders[i].payload.val().amount) -
          ...
          rest =
            ...
            (parseFloat(this.orders[i].payload.val().amount) -
              parseFloat(this.orders[i].payload.val().debt));
          let amount =
            Math.round(this.orders[i].payload.val().amount * 10) / 10;
  ...
  aproveOrder(order: any) {
    this.updateOrder(order.key, { ...order.payload.val(), aproved: true });
  }
      ...
      for (let i = 0; i < this.orders.length; i++) {
        if (
          this.orders[i].payload
            ...
            .clientFantasyName?.toLowerCase()
            .includes(fantasyName.toLowerCase()) &&
          (!inDebt || this.isOrderInDebt(this.orders[i]))
        ) {
  ...
  incrementOrderNumber() {
    ...
    return this.db
      .object('/orderNumber/' + this.orderNumber[0].key)
      .update({ ...this.orderNumber[0].payload.val(), orderNumber: orderNumb });
  }
  ...
  isOrderInDebt(order: any) {
    let today = new Date();
    if (
      Date.parse(today.toString()) - order.payload.val().date >
        TOLERATED_DAYS * 24 * 60 * 60 * 1000 &&
      !order.payload.val().fullPaymentDate
    ) {
      //que pasen los dias tolerados
      return true;
    }
    return false;
  }
    ...
    for (let i = 0; i < payments.length; i++) {
      if (
        payments[i].payload
          ...
          .client.toLowerCase()
          .includes(fantasyName.toLowerCase()) &&
        payments[i].payload.val().date > payment.payload.val().date
      )
        payment = payments[i];
    }
    return payment;
      ...
      for (let i = 0; i < this.clients.length; i++) {
        if (
          this.clients[i].payload
            .val()
            .fantasyName.toLowerCase()
            .includes(clientFantasyName.toLowerCase())
        ) {
  ...
  createPayment(payment: any, clients: any) {
    ...
    this.reorderPaymentDates(payment.client);
    return this.db.list('/payments').push(payment);
  }

  saveDeleteOperation(deletedOperation: any) {
    console.log('deletedOperation', deletedOperation);
    return this.db.list('/deleted').push(deletedOperation);
  }

  reorderPaymentDates(client: string) {
    let payments = this.getClientPayments(client);
    let orders = this.getClientOrders(client);
    if (payments.length == 0 || orders.length == 0) return;
    let paymentsCount = 0;
    let ordersCount = 0;
    let paymentsSum = +payments[0].payload.val().amount;
    let ordersSum = +orders[0].payload.val().amount;
    ...
    while (paymentsCount < payments.length && ordersCount < orders.length) {
      if (paymentsSum > ordersSum - TOLERATED_DEBT) {
        ...
        if (paymentsCount === payments.length) {
          let clientInDebt =
            Math.floor(
              (Date.now() - payments[paymentsCount - 1].payload.val().date) /
                (1000 * 3600 * 24)
            ) > 14;
          while (ordersCount < orders.length) {
            ...
            delete order.fullPaymentDate;
            this.updateOrder(orders[ordersCount].key, order);
            ordersCount++;
          }
        } else paymentsSum += +payments[paymentsCount].payload.val().amount;
      }
    }
  }
  ...
  getLastPaymentDate(client: string) {
    ...
    this.payments.forEach((pay) => {
      if (
        pay.payload.val().client.toLowerCase().includes(client.toLowerCase())
      ) {
        result =
          pay.payload.val().date > result ? pay.payload.val().date : result;
      }
    });
    return result;
  }
  ...
  setPaymentsToAll() {
    this.clients.forEach((client) => {
      this.reorderPaymentDates(client.payload.val().fantasyName);
    });
  }
  ...
  addSaleToSeller(order: any, amount: any, clientFantasyName: string) {
    if (!this.isDateInThisMonth(parseInt(order.payload.val().date))) {
      ...
      if (category == 'Distribuidor') {
        let updatedWholesalerSalesAdded =
          parseFloat(seller.payload.val().wholesalerSalesAdded) +
          parseFloat(amount);
        this.updateSeller(seller.key, {
          wholesalerSalesAdded: updatedWholesalerSalesAdded,
        });
      } else {
        let updatedRetailSalesAdded =
          parseFloat(seller.payload.val().retailSalesAdded) +
          parseFloat(amount);
        this.updateSeller(seller.key, {
          retailSalesAdded: updatedRetailSalesAdded,
        });
      }
    }
  }
  ...
  calculateFullPaymentDates(sellerName: string) {
    for (let k = 0; k < this.clients.length; k++) {
      ...
      let payments = this.sortArrayPayloadByDate(
        this.getClientPayments(this.clients[k].payload.val().fantasyName)
      );
      let orders = this.sortArrayPayloadByDate(
        this.getClientOrders(this.clients[k].payload.val().fantasyName)
      );
      let i = 0;
      let j = 0;
      let ordersAmount = 0;
  ...
  getClientSeller(fantasyName: string) {
    let result = '';
    this.clients.forEach((client: any) => {
      if (client.payload.val().fantasyName.includes(fantasyName)) {
        result = client.payload.val().designatedSeller;
      }
    });
    return result;
  }
  ...
  getSellerOrders(sellerName: string, fantasyName: string) {
    ...
    if (this.orders) {
      for (let i = 0; i < this.orders.length; i++) {
        if (
          this.orders[i].payload.val().order.sellerName == sellerName &&
          this.orders[i].payload
            .val()
            .clientFantasyName.toLowerCase()
            .includes(fantasyName.toLowerCase())
  ...
  getSellerPayments(sellerName: string, fantasyName: string) {
    ...
    if (this.payments[0]) {
      for (let i = 0; i < this.payments.length; i++) {
        if (
          this.payments[i].payload.val().sellerName == sellerName &&
          this.payments[i].payload
            .val()
            .client.toLowerCase()
            .includes(fantasyName.toLowerCase())
  ...
  getDebt(clientFantasyName: string) {
    ...
    this.orders.forEach((order) => {
      if (
        ...
        if (
          keepAdding &&
          today - order.payload.val().date >
            TOLERATED_DAYS * 24 * 60 * 60 * 1000 &&
          !order.payload.val().fullPaymentDate
        )
          result += parseFloat(order.payload.val().amount);
        else;
        if (
          totalPayments < amounts - TOLERATED_DEBT &&
          !order.payload.val().fullPaymentDate &&
          result == 0 &&
          today - order.payload.val().date >
            TOLERATED_DAYS * 24 * 60 * 60 * 1000
