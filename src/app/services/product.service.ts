...
export class ProductService implements OnInit {
  ...
  create(product: any) {
    this.subscription4 = this.getAllRecharges().subscribe((recharges) => {
      ...
      product.price1 =
        product.buyPrice *
        (1 + this.recharges[0].payload.val().distRecharge / 100);
      product.price2 =
        product.buyPrice *
        (1 + this.recharges[0].payload.val().comRecharge / 100);
      product.price3 =
        product.buyPrice *
        (1 + this.recharges[0].payload.val().kiosRecharge / 100);
      product.price4 =
        product.buyPrice *
        (1 + this.recharges[0].payload.val().gymRecharge / 100);
    ...
    for (let i = 0; i < prods.length; i++) {
      for (let j = 0; j < thisProds.length; j++) {
        if (prods[i].product.title == thisProds[j].payload.val().title) {
          ...
          if (add)
            quantity =
              parseInt(thisProds[j].payload.val().stock) +
              parseInt(prods[i].quantity);
          else
            quantity =
              parseInt(thisProds[j].payload.val().stock) -
              parseInt(prods[i].quantity);
          this.db.object('/products/' + thisProds[j].key).update({
            ...thisProds[j].payload.val(),
            stock: quantity,
          });
        }
      }
    ...
    for (let i = 0; i < order.payload.val().order.products.length; i++) {
      for (let j = 0; j < products.length; j++) {
        if (
          ...
          products[j].payload.val().title
        )
          await this.db.object('/products/' + products[j].key).update({
            ...products[j].payload.val(),
            stock:
              parseInt(products[j].payload.val().stock) +
              parseInt(order.payload.val().order.products[i].quantity),
          });
      }
    }
  }
