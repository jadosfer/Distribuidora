...
export class CommissionsComponent implements OnInit {
  ...
  ngOnInit() {
    this.auth.appUser$.subscribe((appUser) => {
      ...
      this.subscription = this.commissionsService
        .getCommissionsByMonth()
        .subscribe((commissionsByMonth) => {
          this.commissionsByMonth = commissionsByMonth;
          this.clearAndOrderCommissionsByMonth(this.commissionsByMonth);
        });
    });
  }

  clearAndOrderCommissionsByMonth(commissionsByMonth: any[]) {
    ...
    for (let i = 1; i < commissionsByMonth.length; i++) {
      if (
        commissionsByMonth[i].payload.val().month == lastMonth &&
        commissionsByMonth[i].payload.val().year == lastYear
      ) {
        console.log('cleaning 1 repited');
        this.db
          .object('/commissionsByMonth/' + commissionsByMonth[i].key)
          .remove();
      } else {
    ...
    this.commissionsByMonthToShow = this.commissionsByMonth.reverse();
    setTimeout(()=>{
