...
export class CommissionsDashComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.subscription = this.catService
      ...
      .subscribe((prodsCategories) => {
        this.prodsCategories = prodsCategories;
        this.subscription2 = this.commissionsService
          .getCommissions()
          .subscribe((commissions) => {
            ...
            for (let i = 0; i < this.prodsCategories.length; i++) {
              rewardsGoalsArray.push(
                new FormControl(
                  this.commissions[0].payload.val().rewardsGoals[i]
                )
              ); //valor de inicializacion
              rewardsArray.push(
                new FormControl(this.commissions[0].payload.val().rewards[i])
              ); //valor de inicializacion
            }
            this.rewardsGoalsArray = new FormArray(rewardsGoalsArray);
            this.rewardsArray = new FormArray(rewardsArray);
            this.myGroup = new FormGroup({
              retailPercent: new FormControl(
                this.commissions[0].payload.val().retailPercent
              ),
              wholesalerPercent: new FormControl(
                this.commissions[0].payload.val().wholesalerPercent
              ),
              monthlyRate: new FormControl(
                this.commissions[0].payload.val().monthlyRate
              ),
              minRetailTotalSales: new FormControl(
                this.commissions[0].payload.val().minRetailTotalSales
              ),
              ...
              .subscribe((orders) => {
                ...
                this.subscription4 = this.auth.appUser$.subscribe((appUser) => {
                  ...
                  if (
                    ...
                    !this.ordersService.fullPaymentDateSet
                  ) {
                    //setTimeout(() => {
                    ...
                    this.ordersService.fullPaymentDateSet = true;
                    //}, 4000);
                  }
                  ...
                  this.ordersService.getAllSellers().subscribe((sellers) => {
                    ...
                    if (
                      ...
                      if (
                        this.commissions[0].payload.val()
                          .monthCommissionsSavedDate.month != today.getMonth()
                      ) {
                        //|| this.commissions[0].payload.val().monthCommissionsSavedDate.year != today.getFullYear()
                        this.month = today.getMonth();
                        this.year = today.getFullYear();
                        this.lastFullMonthOrders =
                          this.getLastFullMonthOrders();
                        this.saveCommissionsByMonth();
                        setTimeout(() => {
                          console.log('salvando comisiones');
                        }, 1000);
                      }
                    }
  ...
  isSellerActive(name: string) {
    for (let i = this.ordersService.orders.length - 1; i > 0; i--) {
      ...
      let day = d.getDate();
      if (
        this.ordersService.orders[i].payload.val().order.sellerName == name &&
        timeAfterOrder <= day + 1
      ) {
        return true;
      }
    }
  ...
  retailSalesPMonth(seller: string, orders: any[]) {
    //if (seller.includes('Nata')) debugger;
    let amount = 0;
    if (orders) {
      for (let i = 0; i < orders.length; i++) {
        if (
          //orders[i].payload.val().order.sellerName.trim() == seller.trim() &&
          this.ordersService
            .getClientSeller(orders[i].payload.val().clientFantasyName)
            .trim() == seller.trim() &&
          this.ordersService.getClientCategory(
            orders[i].payload.val().clientFantasyName
          ) != 'Distribuidor'
        ) {
          amount +=
            parseFloat(orders[i].payload.val().amount) /
            (1 + parseFloat(orders[i].payload.val().iva) / 100);
        }
      }
    }
  ...
  wholesalerSalesPMonth(sellerName: string, orders: any[]) {
    ...
    for (let i = 0; i < orders.length; i++) {
      if (
        orders[i].payload.val().order.sellerName == sellerName &&
        this.ordersService.getClientCategory(
          orders[i].payload.val().clientFantasyName
        ) == 'Distribuidor'
      ) {
    ...
    if (orders) {
      for (let i = 0; i < orders.length; i++) {
        if (orders[i].payload.val().order.sellerName == sellerName) {
          for (
            let j = 0;
            j < orders[i].payload.val().order.products.length;
            j++
          ) {
            if (
              orders[i].payload.val().order.products[j].product.prodsCategory ==
              category
            )
              sales +=
                parseFloat(
                  orders[i].payload.val().order.products[j].discountPrice
                ) * orders[i].payload.val().order.products[j].quantity;
          }
        }
      }
    }
  ...
  retailCommission(retailPercent: number, retailSalesPMonth: number) {
    if (
      retailSalesPMonth >= this.commissions[0].payload.val().minRetailTotalSales
    )
      return (
        Math.round(
          (retailSalesPMonth -
  ...
  wholesalerCommission(
    ...
    wholesalerSalesPMonth: number,
    retailSalesPMonth: number
  ) {
    if (
      retailSalesPMonth >= this.commissions[0].payload.val().minRetailTotalSales
    )
  ...
  save(commissions: any) {
    if (
      confirm(
        'Está segur@ que quiere guardar/crear estos valores de comisiones?'
      )
    ) {
      if (commissions) {
  ...
  rewardCalc(
    ...
    let prodCategorySales = this.prodCategorySales(
      orders,
      prodCategory,
      sellerName
    );
    if (
      prodCategorySales >=
        this.commissions[0].payload.val().rewardsGoals[indexx] &&
      this.retailSalesPMonth(sellerName, orders) >=
        this.commissions[0].payload.val().minRetailTotalSales
    )
  ...
  saveCommissionsByMonth() {
    ...
    this.commissionsService.saveCommissionsByMonth(
      monthCommissions,
      this.commissions
    );
  }
  ...
  getSellersCommissionsInfo() {
    ...
    for (let i = 0; i < this.sellers.length; i++) {
      let retailSalesPMonth = this.retailSalesPMonth(
        this.sellers[i].payload.val().name,
        this.lastFullMonthOrders
      );
      let retailCommission = this.retailCommission(
        this.commissions[0].payload.val().retailPercent,
        retailSalesPMonth
      );
      let wholesalerSalesPMonth = this.wholesalerSalesPMonth(
        this.sellers[i].payload.val().name,
        this.lastFullMonthOrders
      );
      let wholesalerCommission = this.wholesalerCommission(
        this.commissions[0].payload.val().wholesalerPercent,
        wholesalerSalesPMonth,
        retailSalesPMonth
      );
      let sellerPenalty = this.commissionsService.getSellerPenalty(
        this.commissions[0].payload.val().monthlyRate,
        this.sellers[i].payload.val().name,
        this.ordersService.orders
      );
      let rewardsObject = this.getProdCategoryRewards(
        this.sellers[i].payload.val().name
      );
      let prodCategoryRewards = rewardsObject.prodCategoryRewards;
      let totalRewards = rewardsObject.totalRewards;
      let totalIncome =
        retailCommission + wholesalerCommission + totalRewards - sellerPenalty;
      if (this.hasSellerSalesInLastMonth(this.sellers[i].payload.val().name)) {
        sellersCommissionsInfo.push({
          ...
          totalSellerDebtDelayed:
            this.commissionsService.totalSellerDebtDelayed,
        });
      }
    }
  ...
  getLastFullMonthOrders() {
    ...
    for (let i = 0; i < this.ordersService.orders.length; i++) {
      ...
      var date = new Date(
        this.ordersService.orders[i].payload.val().fullPaymentDate
      );
  ...
  getProdCategoryRewards(sellerName: string) {
    ...
    for (let j = 0; j < this.prodsCategories.length; j++) {
      totalRewards += parseFloat(
        this.rewardCalc(
          j,
          this.prodsCategories[j].payload.val().name,
          sellerName,
          this.lastFullMonthOrders
        )
      );
      ...
      if (
        this.retailSalesPMonth(sellerName, this.lastFullMonthOrders) >
          this.commissions[0].payload.val().minRetailTotalSales &&
      ...
      let categoryAim = '0';
      this.commissions[0].payload.val().rewardsGoals[j]
        ? (categoryAim = this.commissions[0].payload.val().rewardsGoals[j])
        : '0';
      prodCategoryRewards.push({
        prodCategory: this.prodsCategories[j].payload.val().name,
        prodCategorySales: this.prodCategorySales(
          this.lastFullMonthOrders,
          this.prodsCategories[j].payload.val().name,
          sellerName
        ),
